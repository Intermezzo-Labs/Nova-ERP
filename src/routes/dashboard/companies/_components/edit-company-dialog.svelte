<script lang="ts">
	...
	import { type UpdateCompanyForm, updateCompanyFormSchema } from '$lib/schemas/company';
...
{#snippet header()}
	<Dialog.Header>
		{#if archived}
			<Dialog.Title>{data.details.name} is archived</Dialog.Title>
			<Dialog.Description>
				Unarchive this company if you want to make any updates.
			</Dialog.Description>
		{:else}
			<Dialog.Title>Edit {data.details.name}</Dialog.Title>
			<Dialog.Description>
