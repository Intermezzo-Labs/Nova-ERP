...
CREATE TABLE IF NOT EXISTS public.product (
    ...
    line_item_id BIGINT REFERENCES public.line_item NOT NULL,
...
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE FORMAT(
            'CREATE POLICY "Users can view own data" ON public.product
                FOR SELECT
...
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE FORMAT(
            'CREATE POLICY "Users can update own data" ON public.product
                FOR UPDATE
                USING (auth.uid() = user_id)'
        );
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE FORMAT(
            'CREATE POLICY "Users can delete own data" ON public.product
                FOR DELETE
                USING (auth.uid() = user_id)'
        );
    END IF;
...
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE FORMAT(
            'CREATE POLICY "Service role can insert" ON public.product
                FOR INSERT
                WITH CHECK (true)'
        );
    END IF;
